...
封装：把数据和操作数据的方法包装在类里，通过访问权限（public、private、protected）来控制对外的暴露。
     提高安全性，隐藏细节，只提供接口，不能直接改变内部数据。

继承：一个类拥有另一个类的属性和行为，可以在原有功能基础上做扩展。增加代码复用性。
     广义的继承有三种实现形式：实现继承、可视继承（GUI 子窗体继承父窗体的属性）、接口继承。  

多态：父类类型的指针或引用指向不同的子类对象。同一接口，不同行为。扩展性强。
    编译时多态-静态多态（函数重载和运算符重载）和运行时多态-动态多态（虚函数和继承）。
